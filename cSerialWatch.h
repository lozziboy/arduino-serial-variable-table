//-------------------------------------------------------------------------------------------------------------------------------------------------------------
// CLASS SERIAL WATCHER
// Class to watch and modify global variables using a pc connected in serial
// 
//  At setup() each variable to be monitored should be mapped through
//   SerialWatcher.Map(F("Name_That_I_Like"), Variable, empty_space(optional));
//
//  At loop() variables should be constantly update through
...
#define SERIALWATCHER_TX_MAX_UINT 0    //max number of transmittable unsigned int: array 0 .. TX_MAX_UINT - 1
#define SERIALWATCHER_TX_MAX_LONG 5    //max number of transmittable long: array 0 .. TX_MAX_LONG - 1
#define SERIALWATCHER_TX_MAX_ULONG 0  //max number of transmittable unsigned long: array 0 .. TX_MAX_ULONG - 1
...
#define SERIALWATCHER_TX_MAX_FLASHSTRING 0 //maxum number of transmittable flashstring: array 0 .. TX_MAX_FLASHSTRING - 1
...
bool cSerialWatcher::IsTextValid(const __FlashStringHelper *Text) {  //true: text is valid | false: text is invalid
  if ( Common.StringSearchSingleChar(Text, SERIALWATCHER_TX_FIELD_SEPARATOR) or 
       Common.StringSearchSingleChar(Text, SERIALWATCHER_TX_CODE_SEPARATOR) or 
       Common.StringSearchSingleChar(Text, SERIALWATCHER_TX_CODE_STREAM_EMPTYSPACE)) { 
    return false;
  }
  else {
    return true;
